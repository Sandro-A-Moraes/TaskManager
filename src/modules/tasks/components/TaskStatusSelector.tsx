...
const TaskStatusSelector = ({ currentStatus, onChangeStatus }: TaskStatusSelectorProps) => (
  <div className='flex justify-around gap-3'>
    <StatusButton current={currentStatus} button='pendente' onClick={() => onChangeStatus('pendente')} label='Pendente' />
    <StatusButton current={currentStatus} button='progresso' onClick={() => onChangeStatus('progresso')} label='Progresso' />
    <StatusButton current={currentStatus} button='concluida' onClick={() => onChangeStatus('concluida')} label='Concluída' />
  </div>
)
